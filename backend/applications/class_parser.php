...
class postParser
{
    ...
    public $settings = array(
        'bburl' => 'https://prototype.imperfectgamers.org/applications/',
        'allowsmilies' => 0,
        'allowmycode' => 1,
        'allowimgcode' => 1,
        'allowvideocode' => 0,
    ...
    public function run_hooks($hook_name, $arguments = array())
    {
        return $arguments;
    }
    public function __construct($settings = array())
    {
        // Override default settings with passed settings
    ...
    {
        // Replace 'forum_post_page.php' with the actual script that displays the post.
        return "https://prototype.imperfectgamers.org/forum_post_page.php?pid={$pid}";
    }
    ...
    function parse_message($message, $options = array())
    {
        ...
        $this->base_url = $this->settings['bburl'];

        // Define the my_substr() if it's not just a simple wrapper for substr()
        if (!function_exists('my_substr')) {
            function my_substr($string, $start, $length = null)
            {
                // Assuming MyBB's my_substr() is a multibyte-safe substring function
                if (function_exists('mb_substr')) {
                    return mb_substr($string, $start, $length, 'UTF-8');
                } else {
                    // Fallback to regular substr if mb_substr is not available
        ...
        if (!function_exists('run_hooks')) {
            function run_hooks($hook_name, $arguments = array())
            {
                return $arguments;
            }
        }

        $message = run_hooks("parse_message_start", $message);
        ...
        $code_matches = array();
        if (!empty($this->options['allow_mycode']) && $this->settings['allowcodemycode'] == 1) {
        ...
        $message = run_hooks("parse_message_htmlsanitized", $message);

        // Replace "me" code and slaps if we have a username
        if (!empty($this->options['me_username']) && $this->settings['allowmemycode'] == 1) {
            global $lang;
        ...
        $message = run_hooks("parse_message_me_mycode", $message);
        ...
        if ($this->settings['allowlinkmycode'] != 1) {
        ...
        $message = run_hooks("parse_message_end", $message);
    ...
    function cache_mycode()
    {
        ...
        if ($this->settings['allowbasicmycode'] == 1) {
        ...
        if ($this->settings['allowsymbolmycode'] == 1) {
        ...
        if ($this->settings['allowemailmycode'] == 1) {
        ...
        if ($this->settings['allowcolormycode'] == 1) {
        ...
        if ($this->settings['allowsizemycode'] == 1) {
        ...
        if ($this->settings['allowalignmycode'] == 1) {
        ...
        if ($this->settings['allowfontmycode'] == 1) {
    ...
    function parse_mycode($message, $options = array())
    {
        ...
        // Reset list cache
        if ($this->settings['allowlistmycode'] == 1) {
    ...
    function cache_smilies()
    {
        ...
        if (is_array($smilies)) {
            ...
            foreach ($smilies as $sid => $smilie) {
                ...
                $smilie['image'] = htmlspecialchars($mybb->get_asset_url($smilie['image']), ENT_QUOTES, 'UTF-8');
                $smilie['name'] = htmlspecialchars($smilie['name'], ENT_QUOTES, 'UTF-8');
...
// Generate a URL to a forum post
function get_post_link($pid) {
    // You'll need to adjust this to match your application's URL structure
    return "https://prototype.imperfectgamers.org/post/{$pid}";
}

        preg_match("#pid=(?:&quot;|\"|')?([0-9]+)[\"']?(?:&quot;|\"|')?#i", $username, $match);
        if (isset($match[1]) && (int) $match[1]) {
            $pid = (int) $match[1];
            $url = $this->settings['bburl'] . "/" . get_post_link($pid) . "#pid$pid";
    ...
    function mycode_parse_video($video, $url)
    {
        ...
        $bbdomain = parse_url($this->settings['bburl'], PHP_URL_HOST);
    ...
    function text_parse_message($message, $options = array())
    {
        ...
        // Run plugin hooks
        $message = run_hooks("text_parse_message", $message);
