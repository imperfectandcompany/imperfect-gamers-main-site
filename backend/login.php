...
if (isset($_POST['login'])) {
    try {
        ...
        if (DatabaseConnector::query('SELECT email from users WHERE email=:email', array(':email' => $emailoruser))) {
            ...
            $profile = DatabaseConnector::query('SELECT user_id FROM profiles WHERE username=:username', array(':username' => $emailoruser));
            if ($profile) {
                // Get user_id from profiles table
                $user_id = $profile[0]['user_id'];

                // Now, check if this user_id exists in the users table and fetch the password
                $user = DatabaseConnector::query('SELECT password FROM users WHERE id=:user_id', array(':user_id' => $user_id));
