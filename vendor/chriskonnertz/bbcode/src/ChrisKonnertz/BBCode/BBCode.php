...
class BBCode
{
    ...
    protected function generateTag(Tag $tag, &$html, Tag $openingTag = null, array $openTags = [])
    {
        ...
        switch ($tag->name) {
            case "quote":
                if ($tag->opening) {
                    // Parse out the author, pid, and dateline from the property
                    preg_match("/'([^']+)' pid='(\d+)' dateline='(\d+)'/", $tag->property, $matches);

                    $author = $matches[1] ?? 'Unknown';    // Default if not provided
                    $pid = $matches[2] ?? 'unknown';       // Default if not provided
